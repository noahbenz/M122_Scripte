...
show_info() {
    ...
    printf '%-45s %s\n' "Aktuelle Systemzeit:" "$(date '+%Y-%m-%d_%H:%M')"
}
...
if [ "$1" == "-f" ]; then
    ...
    hostname=$(uname -n)
    filename=/home/jannfanzun/repo_m122/m122_scripts/auftrag_e/sys.log

    # Informationen sowohl auf dem Terminal als auch in einer Datei anzeigen
    show_info | tee -a "$filename"
else
